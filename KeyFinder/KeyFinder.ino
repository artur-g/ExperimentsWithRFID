...
#define NR_MY_KEYS   39
byte myKeys[NR_MY_KEYS][MFRC522::MF_KEY_SIZE] =  {
...
void loop() {
    ...
    for (byte sector = 15; sector > -1; sector--) {
          ...
          // Try every key per sector
          for (byte keyListIndex = 0; keyListIndex < NR_MY_KEYS; keyListIndex++){
              ...
              for (byte i = 0; i < MFRC522::MF_KEY_SIZE; i++) {
                  key.keyByte[i] = myKeys[keyListIndex][i];
              }
              ...
              status = (MFRC522::StatusCode) mfrc522.PCD_Authenticate(MFRC522::PICC_CMD_MF_AUTH_KEY_A, readBlock, &key, &(mfrc522.uid));
              if (status != MFRC522::STATUS_OK) {
                  Serial.print(F("PCD_Authenticate() failed: "));
                  Serial.println(mfrc522.GetStatusCodeName(status));
                  //return;
              }else{
                Serial.println(F("-------FOUND KEY-------------"));
